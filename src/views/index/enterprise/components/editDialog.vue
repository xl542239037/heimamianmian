<template>
  <!-- 新增对话框 -->
  <el-dialog class="add-dialog" center title="编辑学科" :visible.sync="$parent.eddFormVisible">
    <el-form ref="addForm" :model="addForm" :rules="addFormRules">
      <el-form-item label="学科编号" prop="eid" :label-width="formLabelWidth">
    ...
    <div slot="footer" class="dialog-footer">
      <el-button @click="$parent.eddFormVisible = false">取 消</el-button>
  ...
  methods: {
    submitForm() {
      this.$refs.addForm.validate(valid => {
        if (valid) {
          // 对
          enterpriseEdit(this.addForm).then(res => {
            ...
            if (res.code === 201) {
              ...
              // 关闭弹框
              this.$parent.eddFormVisible = false;
              // 调用父组件的 获取数据方法
              this.$parent.getData();
            }
          });
